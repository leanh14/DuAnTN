        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới đế giày</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/de-giay/them" method="post" modelAttribute="deGiay" class="row">
                        <div class="col-lg-6">
                            ...
                            <c:if test="${trungMa==true}">
                                <p class="text-danger">Mã đế giày đã có trong hệ thống, vui lòng nhập mã mới</p>
                            </c:if>
                        </div>
                        <div class="col-lg-6">
                            <span>Tên đế giày(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                            ...
                            <c:if test="${trungTen==true}">
                                <p class="text-danger">Loại đế giày đã có trong hệ thống, vui lòng nhập tên mới</p>
                            </c:if>
                        </div>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/de-giay/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách đế giày</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/de-giay/hien-thi" method="get">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover no-wrap">
                            ...
                            <tbody>
                            <c:forEach items="${dsDeGiay}" var="deGiay" varStatus="degiay">
                                <tr>
                                    <th scope="row">${degiay.index +1}</th>
                                    <td> ${deGiay.ma}</td>
                                    <td> ${deGiay.ten}</td>
                                    <td> ${deGiay.moTa}</td>
                                        <%--                                    <td> ${chatLieu.nguoiTao}</td>--%>
                                        <%--                                    <td> ${chatLieu.nguoiSua}</td>--%>
                                    <td> ${deGiay.ngayTao}</td>
                                    <td> ${deGiay.ngaySua}</td>
                                    <td> ${deGiay.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/de-giay/view-sua/${deGiay.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/de-giay/xoa/${deGiay.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa dây giày ${dayGiay.ten} ?')">
                                    ...
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                ...
                                                <a class="page-link" href="/de-giay/hien-thi?${pageChoosedNumber-1}" tabindex="-1" aria-disabled="true">Previous</a>
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/de-giay/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test= "${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link" href="/de-giay/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let tenDeGiay = document.getElementById("tenDeGiay").value;
        let maDeGiay = document.getElementById("maDeGiay").value;
        let check = true;

        // kiểm tra mã chất liệu có được nhập không ?
        if (maDeGiay === null || maDayGiay.trim() === "") {
            document.getElementById("messagemaDeGiay").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaDeGiay").innerHTML = "";
        }

        // kiem tra size giay - rong, null, ko phai so, trung du lieu
        if (tenDeGiay === null || tenDeGiay.trim() === "") {
            document.getElementById("messagemaDeGiay").innerHTML = "Vui lòng nhập đế giày";
            return false;
        } else {
            document.getElementById("messagemaDeGiay").innerHTML = "";
        }
