        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới thương hiệu</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/thuong-hieu/them" method="post" modelAttribute="thuongHieu" class="row">
                        <div class="col-lg-6">
                            ...
                            <c:if test="${trungMa==true}">
                                <p class="text-danger">Mã thương hiệu đã có trong hệ thống, vui lòng nhập mã mới</p>
                            </c:if>
                        </div>
                        <div class="col-lg-6">
                            <span>Tên thương hiệu(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                            ...
                            <c:if test="${trungTen==true}">
                                <p class="text-danger">Loại thương hiệu đã có trong hệ thống, vui lòng nhập tên mới</p>
                            </c:if>
                        </div>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/thuong-hieu/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách thương hiệu</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/thuong-hieu/hien-thi" method="get">
...
<%--                                <th scope="col">Sửa bởi</th>--%>
                            ...
                            <tbody>
                            <c:forEach items="${dsThuongHieu}" var="thuongHieu" varStatus="thuonghieu">
                                <tr>
                                    <th scope="row">${thuonghieu.index +1}</th>
                                    <td> ${thuongHieu.ma}</td>
                                    <td> ${thuongHieu.ten}</td>
                                    <td> ${thuongHieu.moTa}</td>

                                    <td> ${thuongHieu.ngayTao}</td>
                                    <td> ${thuongHieu.ngaySua}</td>
                                    <td> ${thuongHieu.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/thuong-hieu/view-sua/${thuongHieu.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/thuong-hieu/xoa/${thuongHieu.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa thương hiệu ${thuongHieu.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/thuong-hieu/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/thuong-hieu/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let maThuongHieu = document.getElementById("maThuongHieu").value;
        let check = true;

        // kiểm tra mã thương hiệu có được nhập không ?
        if (maThuongHieu === null || maThuongHieu.trim() === "") {
            document.getElementById("messagemaThuongHieu").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaThuongHieu").innerHTML = "";
        }

        // kiem tra size giay - rong, null, ko phai so, trung du lieu
        if (tenThuongHieu === null || tenThuongHieu.trim() === "") {
            document.getElementById("messagemaThuongHieu").innerHTML = "Vui lòng nhập thương hiệu";
            return false;
        } else {
            document.getElementById("messagemaThuongHieu").innerHTML = "";
        }
