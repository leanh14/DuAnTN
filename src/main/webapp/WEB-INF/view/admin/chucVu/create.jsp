                    ...
                    <div class="page-header float-right">
                        <div class="page-title">
                            <ol class="breadcrumb text-right">
                                ...
                                <li><a href="#">Quản lý thuộc tính</a></li>
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới chức vụ</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/chuc-vu/them" method="post" modelAttribute="chucVu" class="row">
                        <div class="col-lg-6">
...
<%--                        </div>--%>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/chuc-vu/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách chức vụ</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/chuc-vu/hien-thi" method="get">
...
<%--                                <th scope="col">ID</th>--%>
                            ...
                            <c:forEach items="${dsChucVu}" var="chucVu" varStatus="chucvu">
                                <tr>
                                    <th scope="row">${chucvu.index +1}</th>
                                    <td> ${chucVu.ma}</td>
                                    <td> ${chucVu.ten}</td>
<%--                                    <td> ${chucVu.moTa}</td>--%>
                                    <td> ${chucVu.ngayTao}</td>
                                    <td> ${chucVu.ngaySua}</td>
                                    <td> ${chucVu.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/chuc-vu/view-sua/${chucVu.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/chuc-vu/xoa/${chucVu.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa chức vụ ${chucVu.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/chuc-vu/hien-thi?${pageChoosedNumber-1}"
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/chuc-vu/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/chuc-vu/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let tenChucVu = document.getElementById("tenChucVu").value;
        let maChucVu = document.getElementById("maChucVu").value;
        let check = true;

        // kiểm tra mã chất liệu có được nhập không ?
        if (maChucVu === null || maChatLieu.trim() === "") {
            document.getElementById("messagemaChucVu").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaChucVu").innerHTML = "";
        }

        // kiem tra size giay - rong, null, ko phai so, trung du lieu
        if (tenChucVu === null || tenChucVu.trim() === "") {
            document.getElementById("messagemaChucVu").innerHTML = "Vui lòng nhập chức vụ";
            return false;
        } else {
            document.getElementById("messagemaChucVu").innerHTML = "";
        }
