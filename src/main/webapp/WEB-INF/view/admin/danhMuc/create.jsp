        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới danh mục</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/danh-muc/them" method="post" modelAttribute="danhMuc" class="row">
                        <div class="col-lg-6">
                        ...
                        <div class="col-lg-6">
                            <span>Tên danh mục(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/danh-muc/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách danh mục</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/danh-muc/hien-thi" method="get">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover no-wrap">
                            ...
                            <tbody>
                            <c:forEach items="${dsDanhMuc}" var="danhMuc" varStatus="danhmuc">
                                <tr>
                                    <th scope="row">${danhmuc.index +1}</th>
                                    <td> ${danhMuc.ma}</td>
                                    <td> ${danhMuc.ten}</td>
                                    <td> ${danhMuc.moTa}</td>
                                    ...
                                    <td> ${danhMuc.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/danh-muc/view-sua/${danhMuc.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/danh-muc/xoa/${danhMuc.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa danh mục ${danhMuc.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/danh-muc/hien-thi?${pageChoosedNumber-1}"
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/danh-muc/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/danh-muc/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        if (maDanhMuc === null || maDanhMuc.trim() === "") {
            document.getElementById("messagemaDanhMuc").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaDanhMuc").innerHTML = "";
        }

        // kiem tra size giay - rong, null, ko phai so, trung du lieu
        if (tenDanhMuc === null || tenDanhMuc.trim() === "") {
            document.getElementById("messagemaDanhMuc").innerHTML = "Vui lòng nhập mũi giày";
            return false;
        } else {
            document.getElementById("messagemaDanhMuc").innerHTML = "";
        }
