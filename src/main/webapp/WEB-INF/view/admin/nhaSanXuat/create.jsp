        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới nhà sản xuất</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/nha-sx/them" method="post" modelAttribute="nhaSanXuat" class="row">
                        <div class="col-lg-6">
                        ...
                        <div class="col-lg-6">
                            <span>Tên nhà sản xuất(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/nha-sx/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách nhà sản xuất</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/nha-sx/hien-thi" method="get">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover no-wrap">
                            ...
                            <tbody>
                            <c:forEach items="${dsnhaSanXuat}" var="nhaSanXuat" varStatus="nhasanxuat">
                                <tr>
                                    <th scope="row">${nhasanxuat.index +1}</th>
                                    <td> ${nhaSanXuat.ma}</td>
                                    <td> ${nhaSanXuat.ten}</td>
                                        <%--                                    <td> ${chucVu.moTa}</td>--%>
                                    <td> ${nhaSanXuat.ngayTao}</td>
                                    <td> ${nhaSanXuat.ngaySua}</td>
                                    <td> ${nhaSanXuat.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/nha-sx/view-sua/${nhaSanXuat.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/nha-sx/xoa/${nhaSanXuat.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa nhà sản xuất ${nhaSanXuat.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/nha-sx/hien-thi?${pageChoosedNumber-1}"
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/nha-sx/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/nha-sx/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let tenNhaSanXuat = document.getElementById("tenNhaSanXuat").value;
        let maNhaSanXuat = document.getElementById("maLNhaSanXuat").value;
        let check = true;

        // kiểm tra mã loai san pham có được nhập không ?
        if (maNhaSanXuat === null || maNhaSanXuat.trim() === "") {
            document.getElementById("messagemaNhaSanXuat").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaNhaSanXuat").innerHTML = "";
        }

        // kiem tra nha san xuat - rong, null, ko phai so, trung du lieu
        if (tenNhaSanXuat === null || tenNhaSanXuat.trim() === "") {
            document.getElementById("messagemaNhaSanXuat").innerHTML = "Vui lòng nhập nhà sản xuất";
            return false;
        } else {
            document.getElementById("messagemaNhaSanXuat").innerHTML = "";
        }
