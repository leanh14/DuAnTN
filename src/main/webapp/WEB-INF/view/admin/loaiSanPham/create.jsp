        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới Loại Sản Phẩm</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/loai-sp/them" method="post" modelAttribute="loaiSanPham" class="row">
                        <div class="col-lg-6">
                            ...
                            <c:if test="${trungMa==true}">
                                <p class="text-danger">Mã loại sản phẩm đã có trong hệ thống, vui lòng nhập mã mới</p>
                            </c:if>
                        </div>
                        <div class="col-lg-6">
                            <span>Tên loại sản phẩm(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                            ...
                            <c:if test="${trungTen==true}">
                                <p class="text-danger">Tên loại sản phẩm đã có trong hệ thống, vui lòng nhập tên mới</p>
                            </c:if>
                        </div>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/loai-sp/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách loại sản phẩm</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/loai-sp/hien-thi" method="get">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover no-wrap">
                            ...
                            <tbody>
                            <c:forEach items="${dsLoaiSanPham}" var="loaiSanPham" varStatus="loaisanpham">
                                <tr>
                                    <th scope="row">${loaisanpham.index +1}</th>
                                    <td> ${loaiSanPham.ma}</td>
                                    <td> ${loaiSanPham.ten}</td>
                                        <%--                                    <td> ${chucVu.moTa}</td>--%>
                                    <td> ${loaiSanPham.ngayTao}</td>
                                    <td> ${loaiSanPham.ngaySua}</td>
                                    <td> ${loaiSanPham.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/loai-sp/view-sua/${loaiSanPham.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/loai-sp/xoa/${loaiSanPham.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa loại sản phẩm ${loaiSanPham.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/loai-sp/hien-thi?${pageChoosedNumber-1}"
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/loai-sp/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/loai-sp/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let tenLoaiSanPham = document.getElementById("tenLoaiSanPham").value;
        let maLoaiSanPham = document.getElementById("maLoaiSanPham").value;
        let check = true;

        // kiểm tra mã loai san pham có được nhập không ?
        if (maLoaiSanPham === null || maLoaiSanPham.trim() === "") {
            document.getElementById("messagemaLoaiSanPham").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaLoaiSanPham").innerHTML = "";
        }

        // kiem tra loai san pham - rong, null, ko phai so, trung du lieu
        if (tenLoaiSanPham === null || tenLoaiSanPham.trim() === "") {
            document.getElementById("messagemaLoaiSanPham").innerHTML = "Vui lòng nhập loại sản phẩm";
            return false;
        } else {
            document.getElementById("messagemaLoaiSanPham").innerHTML = "";
        }
