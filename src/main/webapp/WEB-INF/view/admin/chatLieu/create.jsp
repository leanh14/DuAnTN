        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới chất liệu</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/chat-lieu/them" method="post" modelAttribute="chatLieu" class="row">
                        <div class="col-lg-6">
                        ...
                        <div class="col-lg-6">
                            <span>Tên chất liệu(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/chat-lieu/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách chất liệu</h3></strong>
                        </div>
                    </div>
                    <br>
                    <form action="/chat-lieu/hien-thi" method="get">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover no-wrap">
                            ...
                            <tbody>
                            <c:forEach items="${dsChatLieu}" var="chatLieu" varStatus="chatlieu">
                                <tr>
                                    <th scope="row">${chatlieu.index +1}</th>
                                    <td> ${chatLieu.ma}</td>
                                    <td> ${chatLieu.ten}</td>
                                    <td> ${chatLieu.moTa}</td>
                                    <td> ${chatLieu.ngayTao}</td>
                                    <td> ${chatLieu.ngaySua}</td>
                                    <td> ${chatLieu.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/chat-lieu/view-sua/${chatLieu.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/chat-lieu/xoa/${chatLieu.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa mũi giày ${chatLieu.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/chat-lieu/hien-thi?${pageChoosedNumber-1}"
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/chat-lieu/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/chat-lieu/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let maChatLieu = document.getElementById("maChatLieu").value;
        let check = true;

        // kiểm tra mã chất liệu có được nhập không ?
        if (maChatLieu === null || maChatLieu.trim() === "") {
            document.getElementById("messagemaChatLieu").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaChatLieu").innerHTML = "";
        }

        // kiem tra size giay - rong, null, ko phai so, trung du lieu
        if (tenChatLieu === null || tenChatLieu.trim() === "") {
            document.getElementById("messagemaChatLieu").innerHTML = "Vui lòng nhập mũi giày";
            return false;
        } else {
            document.getElementById("messagemaChatLieu").innerHTML = "";
        }
