        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div>
                        <strong class="card-title"><h3>Thêm mới màu sắc</h3></strong>
                    </div>

                </div>
                <div class="card-body">
                    <sf:form action="/mau-sac/them" method="post" modelAttribute="mauSac" class="row">
                        <div class="col-lg-6">
                            ...
                            <c:if test="${trungMa==true}">
                                <p class="text-danger">Mã màu sắc đã có trong hệ thống, vui lòng nhập mã mới</p>
                            </c:if>
                        </div>
                        <div class="col-lg-6">
                            <span>Tên màu sắc(<i class="fa fa-asterisk" style="color: red;"></i>)</span>
                            ...
                            <c:if test="${trungTen==true}">
                                <p class="text-danger">Loại màu sắc đã có trong hệ thống, vui lòng nhập tên mới</p>
                            </c:if>
                        </div>
                        ...
                        <div class="col-lg-4" style="margin-top: 20px; ">
                            ...
                            <a href="/mau-sac/hien-thi" class="btn btn-secondary" style="margin-left: 10px;"> <i
        ...
        <div class="animated fadeIn">
            <div class="card col-lg-12">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <strong class="card-title"><h3>Danh sách màu sắc</h3></strong>
                        </div>
                    </div>
                    <br>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover no-wrap">
                            ...
                            <tbody>
                            <c:forEach items="${dsMauSac}" var="mauSac" varStatus="mausac">
                                <tr>
                                    ...
                                    <td> ${mauSac.trangThai == 1 ? 'Hoạt động': 'Ngưng hoạt động'}</td>
                                    <td><a href="/mau-sac/view-sua/${mauSac.id}" class="btn btn-success"> Xem <i
                                            class="menu-icon fa fa-pencil-square-o"></i> </a></td>
                                    <td><a href="/mau-sac/xoa/${mauSac.id}" class="btn btn-warning"
                                           onclick="return confirm('Bạn chắc chắn muốn xóa mũi giày ${mauSac.ten} ?')">
                                ...
                                <c:if test="${totalPage>0}">
                                    <ul class="pagination justify-content-end">
                                        <c:if test="${pageChoosedNumber > 0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/mau-sac/hien-thi?${pageChoosedNumber-1}"
                                                ...
                                                <c:when test="${pageChoosedNumber==loop.index}">
                                                    <li class="page-item active" aria-current="page">
                                                        <a name="page" class="page-link"
                                                           href="/mau-sac/hien-thi?page=${loop.index}">${loop.index + 1}</a>
                                                    </li>
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <c:if test="${pageChoosedNumber < (totalPage-1)}">
                                            <li class="page-item">
                                                <a class="page-link"
                                                   href="/mau-sac/hien-thi?page=${pageChoosedNumber+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
...
<script>
    ...
    function myValidationFormBeforeAdd() {
        ...
        let maMauSac = document.getElementById("maMauSac").value;
        let check = true;

        // kiểm tra mã chất liệu có được nhập không ?
        if (maMauSac === null || maMauSac.trim() === "") {
            document.getElementById("messagemaMauSac").innerHTML = "Vui lòng nhập mã";
            return false;
        } else {
            document.getElementById("messagemaMauSac").innerHTML = "";
        }

        // kiem tra size giay - rong, null, ko phai so, trung du lieu
        if (tenMauSac === null || tenMauSac.trim() === "") {
            document.getElementById("messagemaMauSac").innerHTML = "Vui lòng nhập màu sắc";
            return false;
        } else {
            document.getElementById("messagemaMauSac").innerHTML = "";
        }
